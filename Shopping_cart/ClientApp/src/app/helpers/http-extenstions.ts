import { ITableState } from '../modules/crud-table';
...
export function baseFilter(entities: any[], requestObj: ITableState) {
  ...
  if (requestObj.sorting.column) {
    entitiesResult = sortArray(
      entitiesResult,
      requestObj.sorting.column,
      requestObj.sorting.direction
    );
  }
  // end
  ...
  const startPosition =
    (requestObj.paginator.page - 1) * requestObj.paginator.pageSize;
...
export function searchInArray(
  ...
  const result = incomingArray.filter((row) => {
    ...
    const keys = Object.keys(row).filter(
      (keyName) => acceptableForSearchTypes[typeof row[keyName]] && row[keyName]
    );
